...
Agent Forwarding for authenticating with services like GitHub. The code and scripts within this guide should be treated 
as reference only -- I do not advise executing anything without first fully understanding the intention and also 
verifying that proper modifications have been made to adapt it to your specific system and needs.
...
5.  Restrict security permissions for the created `id_ed25519` private keys:
    -   Windows Host and/or Clients: 
        ...
        -   See <a
            ...
            target="blank" rel="noreferrer">"User private key files"</a> for more details
...
8.  Ensure the Windows Host's public key has been added to GitHub -- <a
    ...
    target="_blank" rel="noreferrer">instructions for adding keys</a>. You may wish to also add the client public 
    keys if you would like to use them to connect to GitHub directly.
...
10. Import client public keys into `~/.ssh/authorized_keys` (or `%programdata%/ssh/administrators_authorized_keys` if
    ...
    The following import scripts are copied from <a href="https://code.visualstudio.com/docs/remote/troubleshooting"
    target="_blank" rel="noreferrer">VSCode's documentation</a> and will only work for modifying the user owned 
    `~/.ssh/authorized_keys`.
...
11. Restrict security permissions for `authorized_keys`:
    ...
    -   See <a 
        ...
        target="_blank" rel="noreferrer">"authorized_keys"</a> for more details.
    
    If using the `administrators_authorized_keys` file, its permissions should only allow access by `SYSTEM` and the
    `Administrators` group; inheritance should be disabled. See <a 
    ...
    target="_blank" rel="noreferrer">"administrators_authorized_keys"</a> for specifics.
...
2.  Download <a href="https://github.com/jstarks/npiperelay/releases" target="_blank" rel="noreferrer">npiperelay</a> 
    and put `npiperelay.exe` under the Windows portion of the file system. You can then symlink it to some location 
    that's visible to your WSL's *PATH* environment variable.
    ``` bash
...
3.  Add the following to your `~/.bashrc`, sources: <a
    href="https://stuartleeks.com/posts/wsl-ssh-key-forward-to-windows/" target="_blank" rel="noreferrer">
    stuartleeks.com</a>, <a
    href="https://github.com/rupor-github/wsl-ssh-agent#wsl-2-compatibility" target="_blank" rel="noreferrer">
    rupor-github</a>
    ``` bash
