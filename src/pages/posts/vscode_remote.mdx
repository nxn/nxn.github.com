...
for instructions. You may wish to also add the client public keys if you would like to use them to connect to GitHub
directly.
...
the host account you'll be using is designated as an admin and you want to keep the default configuration scheme). The
following import scripts are copied from https://code.visualstudio.com/docs/remote/troubleshooting and will only work
for modifying the user owned `~/.ssh/authorized_keys`.
...
3. Add the following to your `~/.bashrc`, sources: https://stuartleeks.com/posts/wsl-ssh-key-forward-to-windows/,
...
4. Edit VSCode's `settings.json` file and add the following JSON. This will map the Host to the desired platform so that
when connecting you will be able to specify if you want the regular windows environment or the WSL environment contained
within it. 
    ``` json
